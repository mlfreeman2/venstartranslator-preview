> **Historical document.** This integration was developed as "Venstar Translator" (domain `venstar_translator`) inside the [VenstarTranslator monorepo](https://github.com/mlfreeman2/venstartranslator). When it moved to its own repository it was renamed to **Venstar ACC-TSENWIFI Emulator** (domain `venstar_acc_tsenwifi_emulator`). Names and paths below reflect the original development layout and are kept as-is for the historical record.
