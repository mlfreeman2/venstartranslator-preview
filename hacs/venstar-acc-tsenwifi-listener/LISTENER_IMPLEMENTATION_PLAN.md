# Venstar ACC-TSENWIFI Listener — Home Assistant Custom Component Implementation Plan

> **Status: Planned / not yet built.** This document is the design plan for a standalone HACS component that **listens** for Venstar sensor protobuf packets and surfaces them as Home Assistant entities. It is the mirror image of the [Venstar ACC-TSENWIFI Emulator](https://github.com/mlfreeman2/venstar-acc-tsenwifi-emulator) (HA domain `venstar_acc_tsenwifi_emulator`): that one *encodes and broadcasts*; this one *receives and decodes*.
>
> **Not to be confused with the C# app's [`PROTOBUF_LISTENER_PLAN.md`](https://github.com/mlfreeman2/venstartranslator/blob/main/VenstarTranslator/PROTOBUF_LISTENER_PLAN.md)** — that plan adds a diagnostic capture *page to the C# VenstarTranslator app*. Both bind UDP 5001 and decode the same protocol, but they are different deliverables for different audiences: **this component is an end-user product** — someone who owns physical Venstar sensors installs it to pull them into HA — while the C# page is a maintainer-facing troubleshooting tool. If you were asked to implement "the listener plan," confirm which file you were pointed at.
...
- Optional future convergence with the C# app's own listener tool — planned separately in the C# repo's [`PROTOBUF_LISTENER_PLAN.md`](https://github.com/mlfreeman2/venstartranslator/blob/main/VenstarTranslator/PROTOBUF_LISTENER_PLAN.md) — shared protocol understanding, different runtimes.
